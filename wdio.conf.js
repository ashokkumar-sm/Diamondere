...
exports.config = {
    ...
    onComplete: () => {
        ...
        try {
            // Read the JSON report generated by WebDriverIO
            //const jsonFiles = fs.readdirSync(jsonTmpDirectory).filter(file => path.extname(file) === '.json');

            const jsonFiles = fs.readdirSync(jsonTmpDirectory)
            .filter(file => path.extname(file) === '.json')
            .map(jsonFileName => path.join(jsonTmpDirectory, jsonFileName))
            .join(';'); // Use a delimiter of your choice
    

            //const jsonFile = `${jsonTmpDirectory}report.json`;
            const content = JSON.parse(fs.readFileSync(jsonFiles));
    
            // Remove duplicate test cases
            for (let j = 0; j < content.length; j++) {
                const elements = content[j].elements;
                ...
                content[j].elements = uniqueElements;
            }
    
            fs.writeFileSync(jsonFiles, JSON.stringify(content));
